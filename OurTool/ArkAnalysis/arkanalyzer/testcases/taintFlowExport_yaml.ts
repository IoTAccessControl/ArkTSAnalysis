...
// YAML outputs (for partner-friendly review)
const OUT_FLOW_SOURCE_YAML = 'testcases/OUTPUT/callgraph_source_to_sink.yaml';
const OUT_FLOW_ENTRY_YAML = 'testcases/OUTPUT/callgraph_entry_to_sink.yaml';

// code snippet window (lines before/after callsite)
...
const INCLUDE_SINK_ONLY_PATHS = true; // include sink-only nodes when no start reaches them (coverage)
...
function writeText(outPath: string, text: string) {
  fs.mkdirSync(path.dirname(outPath), { recursive: true });
  fs.writeFileSync(outPath, text, 'utf-8');
  console.log(`[OK] TEXT written: ${outPath}`);
}
...
function resolveReadablePath(p0: string): string | null {
  if (!p0) return null;
  const cands: string[] = [];
  cands.push(p0);
  if (!path.isAbsolute(p0)) cands.push(path.join(process.cwd(), p0));
  // try stripping leading project segments if any (best-effort)
  const idx = p0.indexOf('src/');
  if (!path.isAbsolute(p0) && idx > 0) cands.push(path.join(process.cwd(), p0.slice(idx)));
...
function buildSnippetFromFile(filePath: string, line: number, fallbackText?: string): string {
  const resolved = resolveReadablePath(filePath);
  if (!resolved || line <= 0) {
    const t = (fallbackText ?? '').trim();
    if (t) return `${line > 0 ? line : ''} ${t}`.trim();
    return `// (source unavailable) ${filePath}:${line}`;
  }
  ...
  try {
    ...
    const start = Math.max(1, line - SNIPPET_CONTEXT_BEFORE);
    const end = Math.min(lines.length, line + SNIPPET_CONTEXT_AFTER);
    const pad = String(end).length;

    const out: string[] = [];
    for (let ln = start; ln <= end; ln++) {
      const body = (lines[ln - 1] ?? '').replace(/\s+$/g, '');
      out.push(`${String(ln).padStart(pad, ' ')} ${body}`);
    }
    return out.join('\n');
  } catch {
    const t = (fallbackText ?? '').trim();
    if (t) return `${line} ${t}`;
    return `// (read failed) ${filePath}:${line}`;
  }
}

function inferTargetVariableFromText(text: string): string {
  const t = (text ?? '').toString();
  const m = t.match(/\((.*)\)/);
  if (!m) return 'UNKNOWN';
  const inside = m[1];
  // split args (very naive; good enough for "a,b,c" style)
  const args = inside.split(',').map(x => x.trim()).filter(Boolean);
  if (args.length === 0) return 'UNKNOWN';
  // often the "data" is the last arg for write/send/log
  const candidate = args[args.length - 1];
  return candidate.replace(/[);]+$/g, '').trim() || 'UNKNOWN';
}

type YamlHop = { filePath: string; line: number; type?: string; code: string; api?: string; apiId?: string; apiSignature?: string; step?: string };

function buildYamlForFlow(flow: FlowOutput, allNodes: NodeJson[], allEdges: EdgeJson[], startDetails: StartDetail[]): string {
  ...
  const startDetailById = new Map<string, StartDetail>();
  for (const d of startDetails) startDetailById.set(d.id, d);

  const lines: string[] = [];
  for (const p of flow.paths) {
    const sNode = nodeById.get(p.start);
    const kNode = p.sink ? nodeById.get(p.sink) : undefined;
    const isSinkOnly = (p as any).pathType === 'sink_only';

    let targetVar = 'N/A';
    if (!isSinkOnly && p.sink && p.edges.length) {
      const lastE = edgeById.get(p.edges[p.edges.length - 1]);
      const cs = lastE?.callSites?.find(x => (x?.text ?? '').trim()) ?? null;
      if (cs) targetVar = inferTargetVariableFromText(cs.text);
    }

    lines.push(`- sink_api:`);
    if (kNode) {
      lines.push(`    id: ${yamlQuote(kNode.id)}`);
      lines.push(`    signature: ${yamlQuote(kNode.signature)}`);
      if (kNode.taint?.sinkRules?.length) lines.push(`    rules: [${kNode.taint.sinkRules.map(yamlQuote).join(', ')}]`);
    } else {
      lines.push(`    id: null`);
      lines.push(`    signature: null`);
    }

    lines.push(`  source_api:`);
    if (isSinkOnly) {
      lines.push(`    id: null`);
      lines.push(`    signature: null`);
    } else if (sNode) {
      lines.push(`    id: ${yamlQuote(sNode.id)}`);
      lines.push(`    signature: ${yamlQuote(sNode.signature)}`);
      if (sNode.taint?.sourceRules?.length) lines.push(`    rules: [${sNode.taint.sourceRules.map(yamlQuote).join(', ')}]`);
    } else {
      lines.push(`    id: ${yamlQuote(p.start)}`);
      lines.push(`    signature: null`);
    }

    lines.push(`  target_variable: ${yamlQuote(targetVar)}`);
    if ((p as any).pathType) lines.push(`  path_type: ${yamlQuote((p as any).pathType)}`);
    if ((p as any).note) lines.push(`  note: ${yamlQuote((p as any).note)}`);

    const pathType = String((p as any).pathType ?? '');

    const hopKey = new Set<string>();
    const hops: YamlHop[] = [];

    const sd = startDetailById.get(p.start);
    if (sd?.uiOrigins?.length) {
      for (const o of sd.uiOrigins) {
        for (const cs of o.callSites ?? []) {
          const key = `${cs.file}|${cs.line}|${cs.text}`;
          if (hopKey.has(key)) continue;
          hopKey.add(key);
          const apiName = `${o.uiApi.className}.${o.uiApi.methodName}`;
          hops.push({
            filePath: cs.file,
            line: cs.line,
            type: cs.type,
            api: apiName,
            apiId: o.uiApi.nodeId,
            apiSignature: o.uiApi.signature,
            code: buildSnippetFromFile(cs.file, cs.line, cs.text),
          });
        }
      }
    }
    ...
    for (const eid of p.edges) {
      ...
      if (!e?.callSites?.length) continue;
      const cs = e.callSites.find(x => x && x.file && x.line > 0) ?? e.callSites[0];
      if (!cs) continue;
      const key = `${cs.file}|${cs.line}|${cs.text}`;
      if (hopKey.has(key)) continue;
      hopKey.add(key);
      ...
      const apiName = toNode ? `${toNode.className}.${toNode.methodName}` : undefined;
      ...
      hops.push({
        ...
        api: apiName,
        apiId: toNode?.id,
        apiSignature: toNode?.signature,
        code: buildSnippetFromFile(cs.file, cs.line, cs.text),
      });
    }

    if (hops.length === 0) {
      const fp = sNode?.file ?? 'UNKNOWN';
      const apiName = sNode ? `${sNode.className}.${sNode.methodName}` : undefined;
      hops.push({
        filePath: fp,
        line: -1,
        type: 'indirect',
        api: apiName,
        apiId: sNode?.id,
        apiSignature: sNode?.signature,
        code: `// no callsite captured for this path\n// ${sNode?.signature ?? p.start}`,
      });
    }
    ...
    for (let i = 0; i < hops.length; i++) {
      const h = hops[i];
      lines.push(`    - filePath: ${yamlQuote(h.filePath)}`);
      if (pathType === 'entry_to_sink') lines.push(`      step: ${yamlQuote('step' + String(i + 1))}`);
      if (h.line > 0) lines.push(`      line: ${h.line}`);
      if (h.type) lines.push(`      type: ${yamlQuote(h.type)}`);
      if (h.api) lines.push(`      api: ${yamlQuote(h.api)}`);
      if (h.apiId) lines.push(`      api_id: ${yamlQuote(h.apiId)}`);
      if (h.apiSignature) lines.push(`      api_signature: ${yamlQuote(h.apiSignature)}`);
      lines.push(`      code: |`);
      const codeLines = (h.code ?? '').split(/\r?\n/);
...
function buildFlowCoveringStarts(
  ...
  // Ensure sink coverage (optional): if a sink is never reached by any start, still emit it as a single-node path.
  if (INCLUDE_SINK_ONLY_PATHS && label === 'source_to_sink') {
    for (const sk of sinks) {
      if (coveredSinks.has(sk)) continue;
      paths.push({
        start: sk, // placeholder; see pathType
        sink: sk,
        length: 0,
        nodes: [sk],
        edges: [],
        truncated: false,
        pathType: 'sink_only',
        note: 'no start reached this sink in callgraph',
      });
      // NOTE: do NOT add to keptStarts; this is not an actual start.
    }
  }
...
function main() {
  ...
  // partner-friendly YAML (covers all sources; source-only paths included)
  writeText(OUT_FLOW_SOURCE_YAML, buildYamlForFlow(flowSource, nodes, edges, sourceStartDetails));
  ...
  // partner-friendly YAML
  writeText(OUT_FLOW_ENTRY_YAML, buildYamlForFlow(flowEntry, nodes, edges, entryStartDetails));
