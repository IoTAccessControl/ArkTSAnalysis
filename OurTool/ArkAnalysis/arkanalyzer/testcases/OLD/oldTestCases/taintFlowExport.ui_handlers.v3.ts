...
type FlowOutput = {
  ...
  nodes: NodeJson[];
  edges: EdgeJson[];
  paths: Array<{ start: string; sink: string; length: number; nodes: string[]; edges: string[]; truncated: boolean }>;
};
...
function buildFlowWithPaths(allNodes: NodeJson[], allEdges: EdgeJson[], starts: string[], sinks: string[], label: string): FlowOutput {
...
function main() {
  ...
  // fallback: use the *caller* of UI registration as entry (computed in this run, no stale IDs)
  const callerUiEntryIds = uniq(uiBindings.map(b => b.callerId));


  const uiEntryMode = uiHandlerEntryIds.length > 0 ? 'handler' : 'caller_build_fallback';
  const uiEntriesUsed = uiHandlerEntryIds.length > 0 ? uiHandlerEntryIds : callerUiEntryIds;

  const entryStarts = uniq([...sources, ...uiEntriesUsed]);
  ...
  const flowSource = buildFlowWithPaths(nodes, edges, sources, sinks, 'source_to_sink');
  ...
  const flowEntry = buildFlowWithPaths(nodes, edges, entryStarts, sinks, 'entry_to_sink');
