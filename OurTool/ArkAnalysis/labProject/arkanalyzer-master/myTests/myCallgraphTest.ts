...
// // 按“链式箭头”格式打印调用路径
// function printPrettyCallPaths(grouped: Map<string, Map<string, CallRecord[]>>, prev: Map<string, string | null>) {
//   console.log('\n📌 Pretty Call Paths (可读调用链输出):\n');

//   const visited = new Set<string>(); // 避免重复打印

//   for (const callerMap of grouped.values()) {
//     for (const records of callerMap.values()) {
//       for (const rec of records) {
//         const last = rec.calleeSigStr;
...
//         const path = getCallPath(prev, last);

//         // 打印路径
//         path.forEach((sig, idx) => {
//           const indent = '    '.repeat(idx);
//           if (idx === 0) {
//             console.log(`${simplifySignature(sig)}`);
//           } else {
//             console.log(`${indent}-> ${simplifySignature(sig)}`);
//           }
//         });
