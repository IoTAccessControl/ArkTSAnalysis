...
exports.getRelativeSourcePath = exports.enableObfuscateFileName = exports.handleObfuscatedFilePath = exports.enableObfuscatedFilePathConfig = exports.mangleFilePath = exports.generateobfLoggerPrinter = exports.generateConsumerObConfigFile = exports.printUnobfuscationReasons = exports.printWhitelist = exports.writeUnobfuscationContent = exports.writeObfuscationNameCache = exports.fillNameCache = exports.getArkguardNameCache = exports.handleUniversalPathInObf = exports.clearNameCache = exports.readNameCache = exports.collectResevedFileNameInIDEConfig = exports.ObConfigResolver = exports.MergedConfig = exports.ObOptionsForTest = exports.OptionTypeForTest = void 0;
...
const MemoryDottingDefine_1 = require("../utils/MemoryDottingDefine");
const ProjectCollections_1 = require("../utils/ProjectCollections");
var OptionType;
(function (OptionType) {
    ...
    OptionType[OptionType["ENABLE_LIB_OBFUSCATION_OPTIONS"] = 13] = "ENABLE_LIB_OBFUSCATION_OPTIONS";
    OptionType[OptionType["ENABLE_ATKEEP"] = 14] = "ENABLE_ATKEEP";
    OptionType[OptionType["COMPACT"] = 15] = "COMPACT";
    OptionType[OptionType["REMOVE_LOG"] = 16] = "REMOVE_LOG";
    OptionType[OptionType["REMOVE_COMMENTS"] = 17] = "REMOVE_COMMENTS";
    OptionType[OptionType["PRINT_NAMECACHE"] = 18] = "PRINT_NAMECACHE";
    OptionType[OptionType["PRINT_KEPT_NAMES"] = 19] = "PRINT_KEPT_NAMES";
    OptionType[OptionType["APPLY_NAMECACHE"] = 20] = "APPLY_NAMECACHE";
    OptionType[OptionType["EXTRA_OPTIONS"] = 21] = "EXTRA_OPTIONS";
    OptionType[OptionType["STRIP_LANGUAGE_DEFAULT"] = 22] = "STRIP_LANGUAGE_DEFAULT";
    OptionType[OptionType["STRIP_SYSTEM_API_ARGS"] = 23] = "STRIP_SYSTEM_API_ARGS";
    OptionType[OptionType["KEEP_PARAMETER_NAMES"] = 24] = "KEEP_PARAMETER_NAMES";
    OptionType[OptionType["ENABLE_BYTECODE_OBFUSCATION"] = 25] = "ENABLE_BYTECODE_OBFUSCATION";
    OptionType[OptionType["ENABLE_BYTECODE_OBFUSCATION_DEBUGGING"] = 26] = "ENABLE_BYTECODE_OBFUSCATION_DEBUGGING";
    OptionType[OptionType["ENABLE_BYTECODE_OBFUSCATION_ENHANCED"] = 27] = "ENABLE_BYTECODE_OBFUSCATION_ENHANCED";
    OptionType[OptionType["ENABLE_BYTECODE_OBFUSCATION_ARKUI"] = 28] = "ENABLE_BYTECODE_OBFUSCATION_ARKUI";
...
class ObOptions {
    constructor() {
        ...
        this.enableLibObfuscationOptions = false;
        this.enableAtKeep = false;
        this.enableEtsAnnotation = false;
        ...
        this.applyNameCache = '';
        this.stripLanguageDefault = false;
        this.stripSystemApiArgs = false;
        this.keepParameterNames = false;
    }
    mergeObOptions(other) {
        var _a, _b;
        ...
        this.bytecodeObf.obfArkUI = ((_a = this.bytecodeObf) === null || _a === void 0 ? void 0 : _a.obfArkUI) || ((_b = other.bytecodeObf) === null || _b === void 0 ? void 0 : _b.obfArkUI);
...
class MergedConfig {
    ...
    serializeMergedConfig() {
        ...
        if ((_a = this.options.bytecodeObf) === null || _a === void 0 ? void 0 : _a.obfArkUI) {
...
class ObConfigResolver {
    constructor(projectConfig, printObfLogger, isTerser) {
        this.needConsumerConfigs = false;
    ...
    resolveObfuscationConfigs() {
        ...
        (0, PrinterUtils_1.startFilesEvent)(ArkObfuscator_1.EventList.RESOLVE_OBFUSCATION_CONFIGS);
        ...
        let needDependencyConfigs = enableObfuscation || this.needConsumerConfigs;
        this.dependencyConfigs = new MergedConfig();
        const dependencyMaxLength = Math.max(sourceObConfig.dependencies.libraries.length, sourceObConfig.dependencies.hars.length, (_b = (_a = sourceObConfig.dependencies.hsps) === null || _a === void 0 ? void 0 : _a.length) !== null && _b !== void 0 ? _b : 0, (_d = (_c = sourceObConfig.dependencies.hspLibraries) === null || _c === void 0 ? void 0 : _c.length) !== null && _d !== void 0 ? _d : 0);
        ...
        const mergedConfigs = this.getMergedConfigs(selfConfig, this.dependencyConfigs);
        this.handleReservedArray(mergedConfigs);
        (0, PrinterUtils_1.endFilesEvent)(ArkObfuscator_1.EventList.RESOLVE_OBFUSCATION_CONFIGS);
        /**
        ...
        if (needKeepSystemApi && sourceObConfig.obfuscationCacheDir) {
            ...
            if ((0, utils_1.isFileExist)(systemApiCachePath)) {
                (0, PrinterUtils_1.startFilesEvent)(ArkObfuscator_1.EventList.GET_SYSTEM_API_CONFIGS_BY_CACHE);
                this.getSystemApiConfigsByCache(systemApiCachePath);
                (0, PrinterUtils_1.endFilesEvent)(ArkObfuscator_1.EventList.GET_SYSTEM_API_CONFIGS_BY_CACHE);
            }
            else {
                const recordInfo = ArkObfuscator_1.ArkObfuscator.recordStage(MemoryDottingDefine_1.MemoryDottingDefine.SCAN_SYS_API);
                ...
                ArkObfuscator_1.ArkObfuscator.stopRecordStage(recordInfo);
            }
        }
    ...
    emitConsumerConfigFiles() {
        if (this.needConsumerConfigs) {
            let selfConsumerConfig = new MergedConfig();
            (0, PrinterUtils_1.startFilesEvent)(ArkObfuscator_1.EventList.GEN_CONSUMER_CONFIG);
            ...
            (0, PrinterUtils_1.endFilesEvent)(ArkObfuscator_1.EventList.GEN_CONSUMER_CONFIG);
        }
    }
    ...
    handleReservedArray(mergedConfigs) {
        ...
        if (mergedConfigs.options.enablePropertyObfuscation && mergedConfigs.reservedPropertyNames) {
            const propertyReservedInfo = (0, ArkObfuscator_1.separateUniversalReservedItem)(mergedConfigs.reservedPropertyNames, shouldPrintKeptNames);
        ...
        if (mergedConfigs.options.enableToplevelObfuscation && mergedConfigs.reservedGlobalNames) {
            const globalReservedInfo = (0, ArkObfuscator_1.separateUniversalReservedItem)(mergedConfigs.reservedGlobalNames, shouldPrintKeptNames);
    ...
    handleConfigContent(data, configs, configPath) {
        ...
        let extraOptionType = OptionType.NONE;
        let tokenType;
        let dtsFilePaths = [];
        let keepConfigs = [];
        for (let i = 0; i < tokens.length; i++) {
            ...
            const matchedExtraOptions = this.isMatchedExtraOptions(extraOptionType, tokenType, configs);
    ...
    isMatchedExtraOptions(extraOptionType, tokenType, configs) {
    ...
    resolveKeepConfig(keepConfigs, configs, configPath) {
        for (let keepPath of keepConfigs) {
            ...
            if (!fs_1.default.existsSync(tempAbsPath)) {
                const warnInfo = `ArkTS: The path of obfuscation \'-keep\' configuration does not exist: ${keepPath}`;
     ...
     * @param config
     */
    collectSystemApiWhitelist(config, systemApiCachePath) {
        var _a, _b;
        ArkObfuscator_1.ApiExtractor.mPropertySet.clear();
        ArkObfuscator_1.ApiExtractor.mSystemExportSet.clear();
        (0, ApiReader_1.initScanProjectConfigByMergeConfig)(config);
        const sdkApis = (0, utils_1.sortAndDeduplicateStringArr)(this.sourceObConfig.sdkApis);
        let existPreDefineFilePath = '';
        let existArkUIWhitelistPath = '';
        const scannedRootFolder = new Set();
        for (let apiPath of sdkApis) {
            this.collectSdkApiWhitelist(apiPath, scannedRootFolder);
            const preDefineFilePath = path_1.default.join(apiPath, '../build-tools/ets-loader/lib/pre_define.js');
            if (fs_1.default.existsSync(preDefineFilePath)) {
        ...
        const arkUIReservedPropertyNames = this.collectUIApiWhitelist(existPreDefineFilePath, existArkUIWhitelistPath, config);
        let systemApiContent = {};
        if (config.options.enablePropertyObfuscation || ((_a = config.options.bytecodeObf) === null || _a === void 0 ? void 0 : _a.enable)) {
            ...
            const savedNameAndPropertySet = new Set([...ArkObfuscator_1.ApiExtractor.mPropertySet, ...arkUIReservedPropertyNames]);
        ...
        if ((config.options.enableToplevelObfuscation && config.options.enableExportObfuscation) ||
            ((_b = config.options.bytecodeObf) === null || _b === void 0 ? void 0 : _b.enable)) {
        ...
        ArkObfuscator_1.ApiExtractor.mPropertySet.clear();
        ArkObfuscator_1.ApiExtractor.mSystemExportSet.clear();
        (0, ApiReader_1.resetScanProjectConfig)();
    }
    collectSystemApiWhitelistForTest(config, systemApiCachePath) {
        return this.collectSystemApiWhitelist(config, systemApiCachePath);
    }
    collectSdkApiWhitelist(sdkApiPath, scannedRootFolder) {
        ...
        if (!scannedRootFolder.has(componentPath) && fs_1.default.existsSync(componentPath)) {
    ...
    collectPreDefineFile(uiApiPath) {
        ArkObfuscator_1.ApiExtractor.extractStringsFromFile(uiApiPath);
    }
    collectUIApiWhitelist(preDefineFilePath, arkUIWhitelistPath, config) {
        // OptimizedReservedPropertyNames saves accurate list of UI API that need to be kept
        let arkUIWhitelist = { ReservedPropertyNames: [], OptimizedReservedPropertyNames: [] };
        if (fs_1.default.existsSync(arkUIWhitelistPath)) {
        ...
        let arkUIReservedPropertyNames = [...arkUIWhitelist.ReservedPropertyNames, ...arkUIWhitelist.OptimizedReservedPropertyNames];
        if (!config.options.stripSystemApiArgs) {
            if (fs_1.default.existsSync(preDefineFilePath)) {
    ...
    mergeDependencyConfigsByPath(paths, dependencyConfigs) {
    ...
    genConsumerConfigFiles(sourceObConfig, selfConsumerConfig, dependencyConfigs) {
        ...
        this.addKeepConsumer(selfConsumerConfig, CommonCollections_1.AtKeepCollections.keepAsConsumer);
        this.addIntentCollections(selfConsumerConfig, CommonCollections_1.AtIntentCollections);
        selfConsumerConfig.sortAndDeduplicate();
        this.writeConsumerConfigFile(selfConsumerConfig, sourceObConfig.exportRulePath);
    }
    addKeepConsumer(selfConsumerConfig, keepAsConsumer) {
    ...
    addIntentCollections(selfConsumerConfig, intentCollections) {
    ...
    determineNameCachePath(nameCachePath, configPath) {
        if (!fs_1.default.existsSync(nameCachePath)) {
            const errorInfo = `The applied namecache file '${nameCachePath}' configured by '${configPath}' does not exist.`;
            const errorCodeInfo = {
                code: '10804004',
                description: 'ArkTS compiler Error',
...
ObConfigResolver.ENABLE_LIB_OBFUSCATION_OPTIONS = '-enable-lib-obfuscation-options';
ObConfigResolver.ENABLE_ATKEEP = '-use-keep-in-source';
...
ObConfigResolver.ENABLE_BYTECODE_OBFUSCATION = '-enable-bytecode-obfuscation';
ObConfigResolver.ENABLE_BYTECODE_OBFUSCATION_DEBUGGING = '-enable-bytecode-obfuscation-debugging';
ObConfigResolver.ENABLE_BYTECODE_OBFUSCATION_ENHANCED = '-enable-bytecode-obfuscation-enhanced';
ObConfigResolver.ENABLE_BYTECODE_OBFUSCATION_ARKUI = '-enable-bytecode-obfuscation-arkui';
ObConfigResolver.EXTRA_OPTIONS = '-extra-options';
ObConfigResolver.STRIP_LANGUAGE_DEFAULT = 'strip-language-default';
ObConfigResolver.STRIP_SYSTEM_API_ARGS = 'strip-system-api-args';
ObConfigResolver.KEEP_PARAMETER_NAMES = '-keep-parameter-names';
...
function readNameCache(nameCachePath, printObfLogger) {
    try {
        ...
        const errorInfo = `Failed to open ${nameCachePath}. Error message: ${err}`;
        const errorCodeInfo = {
            code: '10804002',
            description: 'ArkTS compiler Error',
...
function clearNameCache() {
    var _a;
    (_a = ArkObfuscator_1.PropCollections.historyMangledTable) === null || _a === void 0 ? void 0 : _a.clear();
    CommonObject_1.nameCacheMap === null || CommonObject_1.nameCacheMap === void 0 ? void 0 : CommonObject_1.nameCacheMap.clear();
    (0, Initializer_1.clearHistoryUnobfuscatedMap)();
}
exports.clearNameCache = clearNameCache;
/**
...
function printWhitelist(obfuscationOptions, nameOptions, defaultPath) {
    ...
    let structSet;
    if (enableProperty) {
        structSet = CommonCollections_1.UnobfuscationCollections.reservedStruct;
    }
    ...
    if (enableAtKeep) {
        let atKeepSet = new Set();
        (0, ProjectCollections_1.addToSet)(atKeepSet, CommonCollections_1.AtKeepCollections.keepAsConsumer.globalNames);
        (0, ProjectCollections_1.addToSet)(atKeepSet, CommonCollections_1.AtKeepCollections.keepAsConsumer.propertyNames);
        (0, ProjectCollections_1.addToSet)(atKeepSet, CommonCollections_1.AtKeepCollections.keepSymbol.globalNames);
        (0, ProjectCollections_1.addToSet)(atKeepSet, CommonCollections_1.AtKeepCollections.keepSymbol.propertyNames);
        whitelistObj.conf.push(...atKeepSet);
    }
    let atIndentSet = new Set();
    (0, ProjectCollections_1.addToSet)(atIndentSet, CommonCollections_1.AtIntentCollections.propertyNames);
    (0, ProjectCollections_1.addToSet)(atIndentSet, CommonCollections_1.AtIntentCollections.globalNames);
...
function printUnobfuscationReasons(configPath, defaultPath) {
    ...
    if (Initializer_1.historyUnobfuscatedPropMap.size === 0) {
    ...
    if (Initializer_1.historyAllUnobfuscatedNamesMap.size === 0) {
        // Full compilation or there is no cache after the previous compilation.
        ...
        let historyAllUnobfuscatedNamesObj = Object.fromEntries(Initializer_1.historyAllUnobfuscatedNamesMap);
        Object.keys(CommonObject_1.unobfuscationNamesObj).forEach(key => {
            historyAllUnobfuscatedNamesObj[key] = CommonObject_1.unobfuscationNamesObj[key];
        });
...
function generateConsumerObConfigFile(obfuscationOptions, printObfLogger) {
    const projectConfig = { obfuscationOptions, compileHar: true };
    let obfLoggerPrinter = generateobfLoggerPrinter(printObfLogger);
    const obConfig = new ObConfigResolver(projectConfig, obfLoggerPrinter);
    obConfig.resolveObfuscationConfigs();
}
exports.generateConsumerObConfigFile = generateConsumerObConfigFile;
function generateobfLoggerPrinter(obfLogger) {
    if (typeof obfLogger === 'function') {
        return obfLogger;
    }
    else {
        return (errorInfo, errorCodeInfo, level) => {
            const isNewLogger = obfLogger.printError && obfLogger.printWarn;
            switch (level) {
                case LoggerLevel.ERROR:
                    isNewLogger ? obfLogger.printError(errorCodeInfo) :
                        obfLogger.error(CommonObject_1.red, errorInfo);
                    return;
                case LoggerLevel.WARN:
                    isNewLogger ? obfLogger.printWarn(errorCodeInfo) :
                        obfLogger.warn(CommonObject_1.yellow, errorInfo);
                    return;
                default:
                    return;
            }
...
exports.generateobfLoggerPrinter = generateobfLoggerPrinter;
...
function enableObfuscatedFilePathConfig(isPackageModules, projectConfig) {
    ...
    const enableBytecodeObfuscation = (_a = hasObfuscationConfig.options.bytecodeObf) === null || _a === void 0 ? void 0 : _a.enable;
