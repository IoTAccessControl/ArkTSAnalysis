...
const ArkObfuscator_2 = require("../../ArkObfuscator");
...
(function (secharmony) {
    ...
    const createRenamePropertiesFactory = function (option) {
        ...
        let shouldPrintKeptNames = !!((_a = option.mUnobfuscationOption) === null || _a === void 0 ? void 0 : _a.mPrintKeptNames);
        ...
        function renamePropertiesFactory(context) {
            ...
            function renamePropertiesTransformer(node) {
                ...
                currentFileType = (0, typescript_1.isSourceFile)(node) ? ArkObfuscator_2.FileUtils.getFileSuffix(node.fileName).ext : undefined;
                const recordInfo = ArkObfuscator_1.ArkObfuscator.recordStage(MemoryDottingDefine_1.MemoryDottingDefine.PROPERTY_OBFUSCATION);
                (0, PrinterUtils_1.startSingleFileEvent)(PrinterTimeAndMemUtils_1.EventList.PROPERTY_OBFUSCATION, ArkObfuscator_1.performancePrinter.timeSumPrinter);
                (0, PrinterUtils_1.startSingleFileEvent)(PrinterTimeAndMemUtils_1.EventList.RENAME_PROPERTIES);
                let ret = renameProperties(node);
                (0, PrinterUtils_1.endSingleFileEvent)(PrinterTimeAndMemUtils_1.EventList.RENAME_PROPERTIES);
                (0, PrinterUtils_1.startSingleFileEvent)(PrinterTimeAndMemUtils_1.EventList.UPDATE_MEMBER_METHOD_NAME);
                ...
                (0, PrinterUtils_1.endSingleFileEvent)(PrinterTimeAndMemUtils_1.EventList.UPDATE_MEMBER_METHOD_NAME);
                (0, PrinterUtils_1.startSingleFileEvent)(PrinterTimeAndMemUtils_1.EventList.SET_PARENT_RECURSIVE);
                let parentNodes = (0, typescript_1.setParentRecursive)(ret, true);
                (0, PrinterUtils_1.endSingleFileEvent)(PrinterTimeAndMemUtils_1.EventList.SET_PARENT_RECURSIVE);
                (0, PrinterUtils_1.endSingleFileEvent)(PrinterTimeAndMemUtils_1.EventList.PROPERTY_OBFUSCATION, ArkObfuscator_1.performancePrinter.timeSumPrinter);
                ArkObfuscator_1.ArkObfuscator.stopRecordStage(recordInfo);
                return parentNodes;
            }
            function renameProperties(node) {
                if (NodeUtils_1.NodeUtils.isObjectLiteralInAnnotation(node, currentFileType)) {
                    return node;
                }
            ...
            function renameProperty(node, computeName) {
                if (!NodeUtils_1.NodeUtils.isPropertyNameType(node)) {
                    return (0, typescript_1.visitEachChild)(node, renameProperties, context);
                }
                ...
                if ((0, TransformUtil_1.isInPropertyWhitelist)(original, CommonCollections_1.UnobfuscationCollections.unobfuscatedPropMap, shouldPrintKeptNames)) {
                    return node;
                }
                let mangledName = original;
                let name = original.startsWith(type_1.annotationPrefix) ? original.substring(type_1.annotationPrefix.length) : undefined;
                if (name) {
            ...
            function getPropertyMangledName(original) {
                let mangledName = getMangledName(original);
                return mangledName;
            }
            function getAnnotationMangledNameWithPrefix(original) {
                let mangledName = `${type_1.annotationPrefix}${getMangledName(original)}`;
                return mangledName;
            }
            function getMangledName(original) {
                ...
                while (!mangledName) {
                    let tmpName = RenameIdentifierTransformer_1.globalGenerator.getName();
