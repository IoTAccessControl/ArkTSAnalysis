 ...
 */
function separateUniversalReservedItem(originalArray, shouldPrintKeptName) {
    if (!originalArray) {
...
function recordWildcardMapping(originString, regExpression, shouldPrintKeptName) {
...
function handleReservedConfig(config, optionName, reservedListName, universalLisName, enableRemove) {
    ...
    if (needSeparate) {
        ...
        const reservedInfo = separateUniversalReservedItem(reservedConfig[reservedListName], !!((_a = config.mUnobfuscationOption) === null || _a === void 0 ? void 0 : _a.mPrintKeptNames));
...
function isReservedLocalVariable(mangledName) {
    ...
    return CommonCollections_1.LocalVariableCollections.reservedLangForLocal.has(mangledName) ||
        ...
        ((_b = CommonCollections_1.UnobfuscationCollections.reservedSdkApiForLocal) === null || _b === void 0 ? void 0 : _b.has(mangledName)) ||
        ((_c = CommonCollections_1.UnobfuscationCollections.reservedExportName) === null || _c === void 0 ? void 0 : _c.has(mangledName));
}
exports.isReservedLocalVariable = isReservedLocalVariable;
function isReservedTopLevel(originalName, enablePropertyObf) {
    var _a, _b, _c;
...
function needToRecordTopLevel(originalName, recordMap, nameWithScope, enablePropertyObf) {
    var _a, _b, _c;
    if (enablePropertyObf) {
...
function isInTopLevelWhitelist(originalName, recordMap, nameWithScope, enablePropertyObf, shouldPrintKeptNames) {
...
function isInPropertyWhitelist(originalName, recordMap, shouldPrintKeptNames) {
    if (shouldPrintKeptNames) {
...
function isInLocalWhitelist(originalName, recordMap, nameWithScope, shouldPrintKeptNames) {
    if (shouldPrintKeptNames) {
