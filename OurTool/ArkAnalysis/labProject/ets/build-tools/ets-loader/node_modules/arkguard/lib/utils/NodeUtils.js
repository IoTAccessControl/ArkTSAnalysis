...
exports.hasExportModifier = exports.collectReservedNameForObf = exports.NodeUtils = void 0;
...
class NodeUtils {
    ...
    static isObjectLiteralInAnnotation(node, currentFileType) {
        if (!node || !node.parent || !(0, typescript_1.isObjectLiteralExpression)(node) || !(0, typescript_1.isCallExpression)(node.parent) || !(0, typescript_1.isDecoratorOrAnnotation)(node.parent.parent)) {
            return false;
        }
        if (currentFileType === ".d.ets" /* Extension.DETS */) {
            return true;
        }
        ...
        if (currentFileType === ".ts" /* Extension.TS */ && name.startsWith(type_1.annotationPrefix)) {
            return true;
        }
        return false;
    }
    ...
    static findSymbolOfIdentifier(checker, node, nodeSymbolMap) {
        var _a;
        let sym = (_a = nodeSymbolMap.get(node)) !== null && _a !== void 0 ? _a : checker.getSymbolAtLocation(node);
    ...
    static isPropertyNameType(node) {
        return (0, typescript_1.isStringLiteralLike)(node) || (0, typescript_1.isIdentifier)(node) || (0, typescript_1.isPrivateIdentifier)(node) || (0, typescript_1.isNumericLiteral)(node);
    }
}
...
function hasExportModifier(node) {
    ...
    const modifiers = (0, typescript_1.canHaveModifiers)(node) ? (0, typescript_1.getModifiers)(node) : undefined;
    if (!modifiers) {
        return false;
    }
    for (const modifier of modifiers) {
        if ((0, typescript_1.isModifier)(modifier) && modifier.kind === typescript_1.SyntaxKind.ExportKeyword) {
            return true;
        }
    }
    return false;
