...
(function (secharmony) {
     ...
     */
    secharmony.nodeSymbolMap = new Map();
    /**
     * type of scope
     */
    let ScopeKind;
    ...
    function createScopeManager() {
        ...
        function analyze(ast, typeChecker, isEnabledExportObfuscation = false, currentFileType) {
            checker = typeChecker;
        ...
        function analyzeScope(node) {
            switch (node.kind) {
                ...
                case typescript_1.SyntaxKind.SourceFile:
                    (0, PrinterUtils_1.startSingleFileEvent)(PrinterTimeAndMemUtils_1.EventList.ANALYZE_SOURCE_FILE);
                    analyzeSourceFile(node);
                    (0, PrinterUtils_1.endSingleFileEvent)(PrinterTimeAndMemUtils_1.EventList.ANALYZE_SOURCE_FILE);
                    break;
                // namespace or module
                ...
                case typescript_1.SyntaxKind.ArrowFunction:
                    (0, PrinterUtils_1.startSingleFileEvent)(PrinterTimeAndMemUtils_1.EventList.ANALYZE_FUNCTION_LIKE);
                    analyzeFunctionLike(node);
                    (0, PrinterUtils_1.endSingleFileEvent)(PrinterTimeAndMemUtils_1.EventList.ANALYZE_FUNCTION_LIKE);
                    break;
                // class like
                case typescript_1.SyntaxKind.ClassExpression:
                case typescript_1.SyntaxKind.ClassDeclaration:
                case typescript_1.SyntaxKind.StructDeclaration:
                    (0, PrinterUtils_1.startSingleFileEvent)(PrinterTimeAndMemUtils_1.EventList.ANALYZE_CLASS_LIKE);
                    analyzeClassLike(node);
                    (0, PrinterUtils_1.endSingleFileEvent)(PrinterTimeAndMemUtils_1.EventList.ANALYZE_CLASS_LIKE);
                    break;
                // for like
                ...
                case typescript_1.SyntaxKind.Identifier:
                    ...
                    if (!(0, typescript_1.isAnnotationPropertyDeclaration)(node.parent)) {
                        analyzeSymbol(node);
                    }
                    break;
                ...
                case typescript_1.SyntaxKind.ObjectLiteralExpression:
                    ...
                    if (!NodeUtils_1.NodeUtils.isObjectLiteralInAnnotation(node, fileType)) {
                ...
                case typescript_1.SyntaxKind.AnnotationDeclaration:
                    analyzeAnnotationDeclaration(node);
                    break;
                default:
        ...
        function analyzeImportNames(node) {
            try {
                ...
                if (exportObfuscation) {
                    ...
                    const nameSymbol = getAndRecordSymbolOfIdentifier(checker, node.name);
                    if (nameSymbol) {
        ...
        function tryAddPropertyNameNodeSymbol(propertyNameNode) {
            ...
            const propertySymbol = getAndRecordSymbolOfIdentifier(checker, propertyNameNode);
            if (!propertySymbol) {
        ...
        function analyzeNamespaceExport(node) {
            ...
            let symbol = getAndRecordSymbolOfIdentifier(checker, node.name);
            if (symbol) {
        ...
        function analyzeFunctionLike(node) {
            ...
            let symbol = getAndRecordSymbolOfIdentifier(checker, nameNode);
            if (symbol) {
        ...
        function analyzeSymbol(node) {
             ...
             */
            if ((0, typescript_1.isParameter)(node.parent) && (0, typescript_1.isFunctionTypeNode)(node.parent.parent)) {
                return;
            }
            let symbol;
            try {
        ...
        function getScopeOfNode(node) {
            ...
            let sym = getAndRecordSymbolOfIdentifier(checker, node);
            if (!sym) {
                return undefined;
            }
        ...
        function analyzeImportEqualsDeclaration(node) {
            ...
            if (hasExport) {
                ...
                let sym = getAndRecordSymbolOfIdentifier(checker, node.name);
                if (sym) {
        ...
        function analyzeAnnotationDeclaration(node) {
            if ((0, NodeUtils_1.hasExportModifier)(node)) {
                current.exportNames.add(node.name.text);
                root.fileExportNames.add(node.name.text);
            }
            let sym = getAndRecordSymbolOfIdentifier(checker, node.name);
        ...
        function tryAddNoSymbolIdentifiers(node) {
            ...
            const sym = getAndRecordSymbolOfIdentifier(checker, node);
            if (!sym) {
                current.mangledNames.add(node.text);
            }
        }
        ...
        function getAndRecordSymbolOfIdentifier(checker, node) {
            const sym = (node && (0, typescript_1.isIdentifier)(node)) ?
                NodeUtils_1.NodeUtils.findSymbolOfIdentifier(checker, node, secharmony.nodeSymbolMap) :
                checker.getSymbolAtLocation(node);
            if (sym) {
                secharmony.nodeSymbolMap.set(node, sym);
            }
            return sym;
        }
    }
